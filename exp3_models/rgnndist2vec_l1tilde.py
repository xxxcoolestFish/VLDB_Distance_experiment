"""
RGNNdist2vec + L̃₁ with Direction-Aware Training (策略三).

在解耦投影头 (策略一) 基础上，加入双向 batch 训练。
利用 d_vu 计算辅助 loss，让 GNN 的 embedding 空间感知方向性。
...
class RGNNdist2vecL1Tilde(nn.Module):
    ...
    def forward(self, x1, x2, embeddings=None):
        ...
        emb1, emb2 = embeddings[x1], embeddings[x2]
    ...
    def subgraph_extraction(self, x1, x2, geometric_data,
        ...
        x1_sub, x2_sub = subgraph_node_map[x1], subgraph_node_map[x2]
    ...
    def _train_step(self, geometric_data, x1, x2, y, criterion, optimizer,
                    num_layers, subgraph_node_map, d_vu=None):
        optimizer.zero_grad()
        ...
        emb1, emb2 = embeddings[x1_sub], embeddings[x2_sub]
        u_sym = self.proj_sym(emb1)
        u_asym = self.proj_asym(emb1)
        v_sym = self.proj_sym(emb2)
        v_asym = self.proj_asym(emb2)
        ...
        for epoch in range(epochs):
            ...
            for batch in dataloader:
                if bidirectional and len(batch) >= 4:
                    ...
                    i, j, d_uv = batch[:3]
                    d_vu = None
                d_uv = d_uv.unsqueeze(-1)
                if d_vu is not None:
                    d_vu = d_vu.unsqueeze(-1)

                i, j, d_uv = i.to(device), j.to(device), d_uv.to(device)
                if d_vu is not None:
                    d_vu = d_vu.to(device)

                loss = self._train_step(
                    self.geometric_data, i, j, d_uv, criterion, optimizer,
                    self.num_layers, subgraph_node_map, d_vu)
                running_loss += loss.item()
                loss_iter_history.append(loss.item())

                if (len(loss_iter_history)) % display_step == 0:
                    ls = f"{loss.item():.8f}" if loss.item() <= 1.0 \
                         else f"{loss.item():.2f}"
                    print(f"Epoch: {epoch+1:>2}/{epochs}, "
                          f"Batch: {(len(loss_iter_history) % len(dataloader))+1:>4}, "
                          f"Loss: {ls:>12}")
