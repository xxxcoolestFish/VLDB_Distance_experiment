"""
Distance-Enhanced GNN (DLGNN-style) 适配道路网络距离估计.

核心思路 (来自 DLGNN, ICML 2021):
    Anchor距离特征 + GNN结构特征 → pairwise距离预测

架构:
    Encoder: GNN(SAGE/GAT) + Anchor距离特征拼接
    Decoder: 三种模式对应三组实验
        实验1: concat → MLP → distance (原版)
        实验2: concat → MLP → y_o,y_d → L1
        实验3: concat → MLP → y_o,y_d -> L1Tilde(r=2,s=62)
"""
...
class DistanceEnhancedGNN(BaseModel):
    """DLGNN-style: anchor距离 + GNN → pairwise距离预测"""
    ...
    def __init__(self, graph, num_nodes, node_attributes, edge_attributes,
                 ...
                 num_anchors=16,  # DLGNN uses anchor nodes for positional encoding
        ...
        # ---- Anchor Distance Features (DLGNN核心) ----
        if num_anchors > 0:
        ...
        # ---- Decoder ----
        # Total input: GNN_u(64) + GNN_v(64) + anchor_u(K) + anchor_v(K)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(), nn.Dropout(0.1),
            nn.Linear(hidden, n_hidden_2 * 2),  # y_o + y_d (128)
        )
    ...
    def forward(self, x1, x2):
        ...
        if self.anchor_feat is not None:
            a1, a2 = self.anchor_feat[x1], self.anchor_feat[x2]
